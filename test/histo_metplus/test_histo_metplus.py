...
#     width_value = int(h.get_config_value('config', 'width'))
#     assert expected_width_value == width_value


def test_base_class_no_item_of_interest(config):
    """ Verify that we are not getting an exception when
    ...
    # Every histogram plot checks for the image_name in the [config] section,
    # and we omitted this key-value in the custom config file histo_metplus_missing_key.conf, so the value will
    # be set to None and the plotting script will proceed to use the default value.
    item_of_interest = 'foo'

    # No longer raise an exception if a section or key-value pair is missing, instead we are setting the key-value
    # pair that is within this section to None, wherein the plot type script will then proceed to use the
    # default value.
    ...
    # Test will fail if an exception or error is raised.  If this is successful, then the test will pass.
    m = MetPlotIni(config_dict, missing_key_conf_dict)
    m.get_config_value('config', item_of_interest)
...
def test_no_section_of_interest(config):
    ...
    # test passes if no errors or exceptions are raised
