...
def config():
    all_default_confs = config_metplotpy.get_all_default_confs()
    config_metplotpy.setup(all_default_confs)
    return config_metplotpy.setup(all_default_confs)



def test_get_all_default_confs():
    """ Verify that we are getting more than 4 default configs (4 from METplus and at least one
        for METplotpy (currently, we only have a histogram_default.conf)
    """
    minimum_number_expected = 5
    all_default_confs = config_metplotpy.get_all_default_confs()
    assert len(all_default_confs) >= minimum_number_expected
