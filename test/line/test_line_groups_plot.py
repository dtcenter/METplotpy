...
def setup():
    ...
    custom_config_filename = f"{cwd}/custom_line_groups.yaml"
...
@pytest.mark.parametrize("test_input,expected",
                         ([f"{cwd}/line_groups.png", True], [f"{cwd}/line_groups.points1", True]))
...
def test_images_match(setup):
    ...
    comparison = CompareImages(f'{cwd}/line_groups_expected.png', actual_file)
    assert comparison.mssim == 1
    cleanup()

@pytest.mark.parametrize("test_input,expected",
                         ([f"{cwd}/intermed_files/line_groups.png", True], [f"{cwd}/intermed_files/line_groups.points1", True]))
def test_files_exist( test_input, expected):
    '''
        Checking that the plot and data files are getting created
    '''
    intermed_dir = f'{cwd}/intermed_files'
    try:
    ...
    os.environ['METPLOTPY_BASE'] = f"{cwd}/../../"
    os.environ['TEST_DIR'] = cwd
    custom_config_filename = f"{cwd}/custom_line_groups2.yaml"
    ...
    try:
        subdir = os.path.join(cwd, intermed_dir)
        plot_file = 'line_groups.png'
        points_file_1 = 'line_groups.points1'
        points_file_2 = 'line_groups.points2'
        html_file = 'line_groups.html'
