...
def collect_files_to_animate(input_dir, forecast_hours, variable, level_type, level, statistic, is_png=True):
    ''' Collect a list of png/jpg files (the only two formats that are supported by the imageio module)
        ...
        will all have the following format: series_Fnnn_[var]_[level]_[stat].png, where
           ...
           :returns sorted_var_level_stat_paths: a list of png or jpg files based on fhr, variable, level type,
                                                 level, and statistic sorted by ascending order
