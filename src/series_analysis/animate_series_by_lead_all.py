...
def collect_files_to_animate(input_dir, forecast_hours, variable, level_type,
                             level, statistic, is_png=True):
    ''' Collect a list of png/jpg files (the only two formats that are supported
        ...
        series_Fnnn_[var]_[level]_[stat].png, where
           :param input_dir: The directory where the png/jpg files to be
           ...
           :param is_png: flag to indicate if this is a png file or jpg file,
                          by default this is set to True, if
                          files to be animated are jpg, set to False
                          Two gif files will be created, one for the specified fhrs,
                          variable, and level for obar statistic and the other for
                          the fbar statistic.
...
def main():
    ...
    statistic = 'obar'
    obar_files = collect_files_to_animate(input_dir, fhrs_list, variable,
    ...
    au.create_gif(animation_duration_secs, obar_files, output_filename)

    # Animate the fbar plots
    statistic = 'fbar'
    fbar_files = collect_files_to_animate(input_dir, fhrs_list, variable,
                                          level_type, level, statistic)
    ...
    au.create_gif(animation_duration_secs, fbar_files, output_filename)
