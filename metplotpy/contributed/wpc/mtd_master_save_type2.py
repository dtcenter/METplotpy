...
#USE ONE MODEL/MEMBER AT A TIME.  THIS SCRIPT IS TYPE TWO OF THE CODE AND IS BEST FOR RETROSPECTIVE
#RUNNING BETWEEN A START DATE AND END DATE. VERSION 1 OF THE CODE AND IS BEST FOR BEING
#RUN IN OPERATIONAL MODE. MJE. 201708-20190606,20210609.
...
#beg_date       = datetime.datetime(2019,5,1,0,0,0)                     #Beginning time of retrospective runs (datetime element)
beg_date       = datetime.datetime(2020,5,1,6,0,0)
end_date       = datetime.datetime(2020,5,1,12,0,0)                     #Ending time of retrospective runs (datetime element)
beg_date       = datetime.datetime(2019,11,15,6,0,0)
end_date       = datetime.datetime(2019,11,15,12,0,0)

#end_date       = datetime.datetime(2021,5,12,6,0,0)                     #Ending time of retrospective runs (datetime element)
...
total_fcst_hrs = 48.0                                                   #Last forecast hour to be considered
total_fcst_hrs = 10.0
...
ops_check      = 60*60*1                                                #If in 'Operational' mode, how many seconds to wait before exiting
...
load_model     = ['HRRRv4_TLE_lag00']                                   #Load model file
...
for dates in np.arange(beg_date_jul,end_date_jul,1):#range(int(round(beg_date_jul)),int(round(end_date_jul))+1): #through the dates
    ...
    if dates == beg_date_jul and dates == end_date_jul:
        init_hrs = range(beg_date.hour,end_date.hour+1,6)
    elif dates == beg_date_jul and dates != end_date_jul:
        init_hrs = range(beg_date.hour,24,6)
    elif dates == end_date_jul and dates != beg_date_jul:
        init_hrs = range(0,end_date.hour+1,6)
    else:
        init_hrs = range(0,24,6)
...
################################################################################
        ...
        if mtd_mode == 'Retro':
            ...
            if (np.sum(hour_success) > 0) and (mtd_mode == 'Retro') and (load_qpe[0] in load_data[model]):
                ...
                print('gzip '+TRACK_PATH+GRIB_PATH_DES+mem[0]+'_'+'{:04d}'.format(datetime_curdate.year)+'{:02d}'.format(datetime_curdate.month)+ \
                    '{:02d}'.format(datetime_curdate.day)+'{:02d}'.format(datetime_curdate.hour)+'_simp_prop'+'_s'+str(int(start_hrs))+\
