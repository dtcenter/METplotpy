...
"""
Program Name: config_metplotpy.py
Contact(s): Minna Win
Abstract:
History Log:  METplotpy version from the original METplus version, config_metplus.py found in the
              METplus Github repository https://github.com/NCAR/METplus
Usage: Used to read the configuration files to setup the METplotpy plot scripts
Parameters: None
Input Files: N/A
Output Files: N/A
"""
...
def setup(baseinputconfs, filename=None, logger=None):
    ...
    and arguments and returns a configuration object."""
    ...
    if logger is None:
        logger = logging.getLogger('metplotpy')

    logger.info('Starting METplotpy configuration setup.')

    # Currently METplotpy command line options are for additional conf file.
    ...
    logger.info('Completed METplotpy configuration setup.')
...
if __name__ == "__main__":
    try:
        ...
        produtil.log.jlogger.critical(
            'config_metplotpy failed: %s' % (str(e),), exc_info=True)
        sys.exit(2)
