...
Incorporated to support the functionality originally provided by the R implementation
used by
METviewer.
 """
...
class TaylorDiagram(BasePlot):
    ...
    def __init__(self, parameters: dict) -> None:
        ...
        self.logger.info(f"Begin Taylor Diagram: {datetime.now()}")
        ...
        self.logger.info(f"Finished creating Taylor diagram: {datetime.now()}")
    ...
    def _read_input_data(self) -> pd.DataFrame:
        """
            ...
            Returns:
                 pandas dataframe representation of the data generated by the MET
                 stat tool.
        """
    ...
    def _create_series(self) -> list:
        """
           Generate all the series objects that are to be plotted.  Each series
           corresponds to
           a permutation of all the series_val_1 variables.

           Args:

               @param input_df:  pandas dataframe representation of the input data
               that was created
                                 by MET.

           Returns:
               a list of series objects (as named tuples) that represent the values
               to be plotted.
        ...
        # use the list of series ordering values to determine how many series objects
        # we need.
    ...
    def _create_figure(self) -> None:
        ...
        self.logger.info("Create the figure.")
        ...
        self.logger.info("Creating positive correlation diagram.")
        pos_correlation_only = self.config_obj.values_of_corr
        if pos_correlation_only:
            self.logger.info("Positive correlation only requested.")
            # positive correlation only
            tick_max = np.pi / 2
        else:
            # create diagram of positive and negative correlations
            self.logger.info("Positive and negative correlations requested.")
            tick_max = np.pi
        ...
        _, = self.ax.plot([0], refstd, marker='o', color="black",
                          markerfacecolor="none", markersize=5, ls=':', label='_')
        t = np.linspace(0, tick_max)
        r = np.zeros_like(t) + refstd
        self.ax.plot(t, r, 'k--', label='_')

        # add RMSE contours
        self.logger.info("Adding RMSE contours.")
        rs, ts = np.meshgrid(np.linspace(stdev_min, stdev_max),
                             np.linspace(0, tick_max))

        # compute centered RMS difference
        self.logger.info("Computing centered RMS difference.")
        rms = np.sqrt(refstd ** 2 + rs ** 2 - 2 * refstd * rs * np.cos(ts))
        levels = 5

        if self.config_obj.show_gamma:
            contours = self.ax.contour(ts, rs, rms, levels, colors="#cccccc",
        ...
        # Make some adjustments to the legend box for METviewer default settings when
        # show_gamma is set to
        # False and only positive correlations are to be shown.
        # The default in METviewer for the y-offset of the bbox is y=-0.25 and when
        # y=-.1,
        # the legend box remains in the plot. An adjustment was alread made: y =
        # default y_offset + 0.15
        # in the taylor_diagram_config.py script.  Add another 0.2 to get the bbox y
        # offset value to .1
