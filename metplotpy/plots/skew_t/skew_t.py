...
def retrieve_temperatures(sounding_data: pd.DataFrame, hour_of_interest: str,
    ...
    for cur_lev in plevs:
        ...
        if temp_hour != 'NA':
            temperatures_by_hour.append(int(temp_hour) / 10)
...
def retrieve_winds(sounding_data: pd.DataFrame, hour_of_interest: str,
                   all_pressure_levels: list, logger) -> tuple:
    '''
...
def calculate_dewpoint(rh: list, temps: list, logger) -> list:
...
def create_skew_t(input_file: str, config: dict, logger: logging) -> None:
    ...
    for cur_time in times_list:
        ...
        dew_pt = calculate_dewpoint(all_rh, temperature, logger)

        # Wind barbs
        u_winds, v_winds = retrieve_winds(sounding_df, cur_time, all_pressure_levels, logger)
        ...
        skew.plot(
            pressure, temperature,  linewidth=temp_linewidth,
