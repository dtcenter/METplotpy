...
Description
===========
A skew-T log P plot is a thermodynamic diagram used for plotting upper air observations. This skew-T log P diagram was developed to read in and the plot the ASCII output of tropical cyclone diagnostic (TC-Diag) sounding data generated by a diagnostics code set developed by Colorado Institute for Research in the Atmosphere (CIRA). The TC-Diag data are currently derived along the projected track of the center of a tropical cyclone and are meant to represent the thermodynamic environment that the storm will be in (according to the model projection bein used). This TC-Diag code set is in the process of being incorporated into the METplus suite and will be available in an upcoming release. This section explains how to create Skew-T plots using METplotpy's Python-based plotter
...
Python script and sample data, generates a Skew-T plot:
