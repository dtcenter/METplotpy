...
according to `yaml.org <https://yaml.org>`_, it is a "human-readable
...
Two configuration files are required, the first is a default configuration
file, **bar_defaults.yaml** that is found in the
...
where the METplotpy source code has been saved.  The second required
YAML configuration file is a user-supplied "custom" configuration file
that is used to customize/override the default
settings in the **bar_defaults.yaml** file.  The custom configuration file
